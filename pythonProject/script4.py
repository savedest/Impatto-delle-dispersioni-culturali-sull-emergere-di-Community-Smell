...
file_path = 'C:\\Users\\saver\\PycharmProjects\\pythonProject\\hofstede.json'
...
for index, row in df_dataset.iterrows():
    ...
    for country in countries:
        ...
        if not match.empty:
            # print(match['pdi'].values[0])
            # pdi_values.append(str(match['pdi'].values[0]))
            # idv_values.append(str(match['idv'].values[0]))
            # mas_values.append(str(match['mas'].values[0]))
            # uai_values.append(str(match['uai'].values[0]))
            # ltowvs_values.append(str(match['ltowvs'].values[0]))
            # ivr_values.append(str(match['ivr'].values[0]))
...
df_dataset.to_excel('deviazione_standard_hofstede_final.xlsx', index=False)
